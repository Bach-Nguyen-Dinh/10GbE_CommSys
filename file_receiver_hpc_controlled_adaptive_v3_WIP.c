...
typedef struct {
    ...
    int sender_socket;               // Socket to send ready signals to sender
    pthread_mutex_t ready_mutex;     // Protect ready signaling
    pthread_cond_t ready_cond;       // Condition variable for signaling
    pthread_t ready_thread;          // Dedicated thread for ready signaling
    volatile int file_processing_done; // Signal when file is done processing
} receiver_ctx_t;
...
static void *ready_signaling_thread(void *arg);
static int connect_to_sender(const char *sender_ip, int port);
static int comprehensive_socket_health_check(int socket_fd);
static int send_ready_signal_bulletproof(receiver_ctx_t *ctx, uint8_t signal, int file_num);
...
static void *adaptive_receiver_thread(void *arg) {
    ...
    // Accept connection (existing code)
    ...
    // FIXED FILE PROCESSING LOOP
    for (int file_num = 1; file_num <= MAX_FILES && running && ctx->test_active; file_num++) {
        data->bytes_received = 0;
        data->total_bytes_processed = 0;
        
        // Only thread 0 reads the header
        if (data->thread_id == 0) {
            // Signal ready for next header (skip for first file)
            if (file_num > 1) {
                printf("Thread 0: Signaling ready for file %d header...\n", file_num);
                
                pthread_mutex_lock(&ctx->ready_mutex);
                ctx->file_processing_done = 1;
                pthread_cond_signal(&ctx->ready_cond);
                pthread_mutex_unlock(&ctx->ready_mutex);
                
                printf("Thread 0: Ready signal queued\n");
            }
            ...
            data->expected_file_size = file_size;
        ...
        // FIXED ADAPTIVE CHUNK RECEIVING LOOP
...
chunk_loop_exit:
        ...
        // THREAD 0: FILE COMPLETION REPORTING
        if (data->thread_id == 0) {
        ...
        // Reset for next file
        ctx->file_processing_done = 0;  // Reset ready signaling flag
    }
...
static void *ready_signaling_thread(void *arg) {
    ...
    uint8_t ready_signal = READY_SIGNAL;
    int file_count = 0;
    
    printf("Bulletproof Ready Signaling Thread Started\n");
    printf("Features: Health Monitoring + ACK Protocol + Retry Logic\n");
    
    // Send initial ready signal for file 1
    if (send_ready_signal_bulletproof(ctx, ready_signal, 1) < 0) {
        printf("FATAL: Could not establish initial ready signaling\n");
        ctx->test_active = 0;
        return NULL;
    }
    
    while (running && ctx->test_active) {
        // Wait for file processing to complete
        pthread_mutex_lock(&ctx->ready_mutex);
        while (!ctx->file_processing_done && running && ctx->test_active) {
            struct timespec timeout;
            clock_gettime(CLOCK_REALTIME, &timeout);
            timeout.tv_sec += 10;  // 10 second periodic check
            
            int wait_result = pthread_cond_timedwait(&ctx->ready_cond, &ctx->ready_mutex, &timeout);
            
            if (wait_result == ETIMEDOUT) {
                // Periodic health check during long waits
                pthread_mutex_unlock(&ctx->ready_mutex);
                
                if (comprehensive_socket_health_check(ctx->sender_socket) < 0) {
                    printf("Health check failed during wait - connection degraded\n");
                }
        ...
        ctx->file_processing_done = 0;
        pthread_mutex_unlock(&ctx->ready_mutex);
        
        // Send bulletproof ready signal
        printf("\nPreparing bulletproof ready signal for File %d\n", file_count + 1);
        
        if (send_ready_signal_bulletproof(ctx, ready_signal, file_count + 1) < 0) {
            printf("CRITICAL FAILURE: Cannot guarantee ready signal delivery\n");
            printf("Options: 1) Continue anyway  2) Abort transfer\n");
            
            // Decision point - continuing with degraded signaling
            printf("Decision: Continuing with degraded signaling...\n");
            // Alternative: ctx->test_active = 0; break;
        }
    }
    
    printf("Bulletproof Ready Signaling Thread Terminated\n");
    return NULL;
}
...
static int comprehensive_socket_health_check(int socket_fd) {
    // Check 1: Socket error status
    int error = 0;
    socklen_t len = sizeof(error);
    if (getsockopt(socket_fd, SOL_SOCKET, SO_ERROR, &error, &len) < 0) {
        printf("Socket health check: getsockopt failed\n");
        return -1;
    }
    if (error != 0) {
        printf("Socket health check: socket error detected: %s\n", strerror(error));
        return -1;
    }
    ...
    if (getsockopt(socket_fd, IPPROTO_TCP, TCP_INFO, &info, &len) == 0) {
        if (info.tcpi_state != TCP_ESTABLISHED) {
            printf("Socket health check: TCP not in ESTABLISHED state (state=%d)\n", info.tcpi_state);
            return -1;
        }
    }
    
    // Check 3: Send test (0 bytes)
    ssize_t result = send(socket_fd, NULL, 0, MSG_DONTWAIT | MSG_NOSIGNAL);
    if (result < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
        printf("Socket health check: send test failed: %s\n", strerror(errno));
        return -1;
    }
...
static int send_ready_signal_bulletproof(receiver_ctx_t *ctx, uint8_t signal, int file_num) {
    int max_attempts = 7;  // Increased attempts for bulletproof delivery
    
    for (int attempt = 0; attempt < max_attempts; attempt++) {
        printf("=== Ready Signal Attempt %d/%d for File %d ===\n", 
               attempt + 1, max_attempts, file_num);
        
        // === PREVENTION LAYER: Health check before attempting ===
        if (comprehensive_socket_health_check(ctx->sender_socket) < 0) {
            printf("Health check failed - socket needs repair\n");
            
            // Close and recreate connection
            close(ctx->sender_socket);
            ctx->sender_socket = connect_to_sender("169.254.207.123", 5403); // Use your sender IP
            
            if (ctx->sender_socket < 0) {
                printf("Failed to recreate connection (attempt %d)\n", attempt + 1);
            ...
            printf("Connection recreated successfully\n");
        }
        
        // === CONFIRMATION LAYER: Send with ACK protocol ===
        printf("Sending ready signal (0x%02X)...\n", signal);
        ssize_t sent = send(ctx->sender_socket, &signal, 1, MSG_NOSIGNAL);
        
        if (sent <= 0) {
            printf("Send failed: %s (attempt %d)\n", strerror(errno), attempt + 1);
            if (attempt < max_attempts - 1) {
                sleep(1);
                continue;
            }
        ...
        printf("Signal sent, waiting for acknowledgment...\n");
        
        // Wait for acknowledgment with timeout
        fd_set read_fds;
        FD_ZERO(&read_fds);
        FD_SET(ctx->sender_socket, &read_fds);
        
        struct timeval timeout;
        timeout.tv_sec = 8;    // Generous timeout
        timeout.tv_usec = 0;
        
        int select_result = select(ctx->sender_socket + 1, &read_fds, NULL, NULL, &timeout);
        
        if (select_result > 0 && FD_ISSET(ctx->sender_socket, &read_fds)) {
            uint8_t ack_signal;
            ssize_t ack_received = recv(ctx->sender_socket, &ack_signal, 1, 0);
            
            if (ack_received > 0) {
                if (ack_signal == ACK_SIGNAL) {
                    printf("READY SIGNAL CONFIRMED for File %d (attempt %d)\n", 
                           file_num, attempt + 1);
                    return 0;  // SUCCESS!
                } else {
                    printf("Invalid acknowledgment received: 0x%02X\n", ack_signal);
                }
            } else {
                printf("Failed to read acknowledgment: %s\n", strerror(errno));
            }
        } else if (select_result == 0) {
            printf("Timeout waiting for acknowledgment (attempt %d)\n", attempt + 1);
        } else {
        ...
        if (attempt < max_attempts - 1) {
            int backoff = (1 << attempt) > 8 ? 8 : (1 << attempt);  // Cap at 8 seconds
            printf("Backing off for %d seconds before retry...\n", backoff);
            sleep(backoff);
        }
    }
    
    printf("CRITICAL: Ready signal delivery FAILED after %d attempts for File %d\n", 
           max_attempts, file_num);
    return -1;
}
...
int main(int argc, char *argv[]) {
    ...
    // Establish connection to sender for ready signaling (use port+100)
    printf("Connecting to sender for ready signaling...\n");
    ctx.sender_socket = connect_to_sender(sender_ip, base_port + 100);
    if (ctx.sender_socket < 0) {
        printf("Failed to connect to sender for signaling\n");
        return 1;
    }
    
    // Start ready signaling thread
    if (pthread_create(&ctx.ready_thread, NULL, ready_signaling_thread, &ctx) != 0) {
        perror("pthread_create ready_thread");
        return 1;
    }

    ctx.file_processing_done = 1;  // Signal ready for first file

    printf("Ready signaling thread started\n");
    ...
    // Wait for ready signaling thread
    pthread_join(ctx.ready_thread, NULL);
    
    // Cleanup ready signaling
    close(ctx.sender_socket);
    pthread_mutex_destroy(&ctx.ready_mutex);
