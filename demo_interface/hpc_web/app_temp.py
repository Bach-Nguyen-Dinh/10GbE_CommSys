...
class HPCFileTransferManager:
    ...
    def scan_rdb_png_files(self):
        ...
        try:
            print(f"Scanning RDB at: {RDB_FLASK_URL}/api/list-files")
            response = requests.get(f"{RDB_FLASK_URL}/api/list-files", timeout=10)            
            
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    all_files = data.get('files', [])
                    files = [f for f in all_files if f.get('is_png', False)]
    ...
    def scan_rdb_tif_files(self):
        ...
        try:
            print(f"Scanning RDB for TIF files at: {RDB_FLASK_URL}/api/scan-tif-files")
            response = requests.get(f"{RDB_FLASK_URL}/api/scan-tif-files", timeout=10)            
            
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    all_files = data.get('file_info', [])
                    print(f"Successfully got {len(all_files)} TIF files from RDB")
                    return all_files
                else:
                    print(f"RDB returned error: {data}")
                    return []
            ...
            print(f"Error scanning RDB for TIF files: {e}")
            return []
    ...
    def monitor_receiver_output(self):
        """Monitor receiver output for FINAL metrics only - progress handled by UDP"""
        ...
        try:
            for line in iter(self.receiver_process.stdout.readline, ''):
                if line:
                    ...
                    # Only parse FINAL average results, ignore individual transfer logs
                    if "=== FINAL AVERAGE RESULTS" in line:
                        print("*** Parsing final average metrics ***")
                        continue
                    
                    # Parse final metrics
                    elif "File size:" in line and "MB" in line:
                    ...
                    # When final metrics are complete, emit completion event
                    if self.transfer_metrics.get('throughput') and self.transfer_metrics.get('duration'):
...
def scan_tif_files():
    ...
    try:
        ...
        files = manager.scan_rdb_tif_files()
        print(f"Returning {len(files)} TIF files")
        return jsonify({
            'success': True,
            'files': files
        })
